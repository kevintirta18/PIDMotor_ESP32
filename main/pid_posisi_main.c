...
static void pid_callback(void* arg)
{
    ...
    float target, error, control;
    uint32_t adc_reading;
    
    // Read set-point
    adc_reading = read_adc();
    //printf("adc: %d, ", adc_reading);
    target = ((float) adc_reading / 4095) * MAX_POSITION;
    ...
    error = target - delta_position;
    error_integral = error_integral + error;
    control = error * 5 + error_integral * 0.5;
