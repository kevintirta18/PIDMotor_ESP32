...
#define CHANGE_TARGET_PERIOD  20000000 // in microseconds
...
static void pid_callback(void* arg)
{
    ...
    float current_velocity, error, control, current_acceleration;
    uint32_t adc_reading;
    
    // Read set-point
    //adc_reading = read_adc();
    //target = ((float) adc_reading / 4095) * 100;
    
    // Compute angular velocity in rpm
    current_velocity = ((float) delta_position / 2400) / ((float) POS_SAMPLING_PERIOD / 1000000) * 60;
    
    current_acceleration = ((float) current_velocity - prev_velocity) /((float) POS_SAMPLING_PERIOD / 1000000);
    prev_velocity = current_velocity;
    // Reset delta_position
    delta_position = 0;
    
    // Compute control signal
    error = target - current_acceleration;
    error_integral = error_integral + error;
    control = error * 3 + error_integral * 0.1;
    ...
    printf("%.2f,", target);
    printf("%.2f,", error);
    printf("%.2f,", current_acceleration);
    printf("%d\n", power);
    
    motor_power(power, &ledc_channel);
}

static void change_target_callback(void* arg)
{
    if (0 == count)
        target = 50;
    else if (1 == count)
...
void app_main(void)
{
    ...
    const esp_timer_create_args_t change_target_args = {
        .callback = &change_target_callback,
    };

    // Configure timer with the given settings
    esp_timer_handle_t pid, change_target;
    esp_timer_create(&pid_args, &pid);
    esp_timer_create(&change_target_args, &change_target);
    
    // Start the timers
    esp_timer_start_periodic(pid, POS_SAMPLING_PERIOD);
    esp_timer_start_periodic(change_target, CHANGE_TARGET_PERIOD);
